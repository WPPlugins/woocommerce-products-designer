        ...
        if (wp_mkdir_p($generation_path)) {
            ...
            if (!empty($result) && is_array($result)) {
                ...
                if ($cart_item_key) {
                    ...
                    $variation = array();
                    if ($variable_product->get_type() == "simple")
...
//            get_currentuserinfo();
    ...
    function get_order_custom_admin_data($item_id, $item, $_product) {
        GLOBAL $wpd_settings;
        $output_options = WPD_Admin::get_proper_value($wpd_settings, "wpc-output-options", array());
        $design_composition_visible = WPD_Admin::get_proper_value($output_options, "design-composition", 'no');
        $output = "";
...
//                        $img_src=$data["image"];
                    ...
                    $output.='<span><a class="modal-trigger button" data-toggle="modal" data-target="#' . $modal_id . '">' . ucfirst($data_key) . '</a></span>';
                    $output.='<div class="modal fade modal wpc_part" id="' . $modal_id . '" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">

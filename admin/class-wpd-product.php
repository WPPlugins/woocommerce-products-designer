            ...
            foreach ($variations as $variation)
            {
                ...
                foreach($attributes as $attribute)
                {
                 ...
                 if($tab_id=='wpc_output_setting_tab_data')
                    ...
                    $this->get_product_tab_data_content_line($variation_id, $attributes_str, $product->get_id());
            }
        }
        else if($product->get_type()=="simple")
        {
             if($tab_id=='wpc_output_setting_tab_data')
                $this->get_product_output_settings($product->get_id(), "Simple product");
            else
                $this->get_product_tab_data_content_line($product->get_id(), "Simple product", $product->get_id());
        }
        else
            echo "<div style='margin:10px; color:red;'>We don't currently support ".$product->get_type()." products.</div>";

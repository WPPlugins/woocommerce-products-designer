...
//                }
        ...
        $product = wc_get_product($post_id);
            ...
            if (isset($wpc_metas['is-customizable']) && !empty($wpc_metas['is-customizable'])) {
                if (isset($wpc_metas['can-design-from-blank'])) {
                    ?><input type="button" value="<?php _e("Design from blank", "wpd"); ?>" data-id="<?php echo $post_id; ?>" data-type="<?php echo $product->get_type(); ?>" class="mg-top-10 wpc-customize-product"/><?php
            }
        }
    }
    ...
    function get_customize_btn_loop($html, $product) {
        ...
        if (isset($wpc_metas['is-customizable']) && !empty($wpc_metas['is-customizable'])) {
            ...
            if ($product->get_type() == "simple" && $design_from_blank)
                $html.='<input type="button" value="' . __("Design from blank", "wpd") . '" data-id="' . $product->id . '" data-type="' . $product->get_type() . '" class="mg-top-10 wpc-customize-product"/>';
        }
        return $html;
    }
